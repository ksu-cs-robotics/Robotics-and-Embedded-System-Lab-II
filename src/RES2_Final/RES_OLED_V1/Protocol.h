...
#define RESET_MSG           'r'
#define ODOM_MSG            'o'  // pose: x, y, orientation  
...
void updateControl(Stream *serialport, char msgBuffer[]);
...
void evaluateMessage(Stream *serialport, char msgBuffer[]){
  if (msgBuffer[3] == VEL_MSG){
    updateControl(serialport, msgBuffer);
...
void updateControl(Stream *serialport, char msgBuffer[]){
  ...
  while (command != 0){
    if (temp == 2){
        serialport->println(String(String(command).toFloat()));
